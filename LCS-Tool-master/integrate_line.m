...
function [position,varargout] = integrate_line(timespan,initialCondition,domain,flowResolution,flowPeriodicBc,vectorField,odeSolverOptions,varargin)
...
vectorXGrid = reshape(vectorField(:,1),fliplr(flowResolution));
vectorYGrid = reshape(vectorField(:,2),fliplr(flowResolution));

vectorInterpolant.x = griddedInterpolant({positionY,positionX},vectorXGrid, 'linear'); %makima ?
vectorInterpolant.y = griddedInterpolant({positionY,positionX},vectorYGrid, 'linear');

initialCondition = apply_periodic_bc(initialCondition,flowPeriodicBc,domain);
...
if ~isempty(poincareSection)
    ...
    if checkDiscontinuity
        continuousInterpolant = is_element_with_orient_discont(initialCondition,domain,flowResolution,vectorXGrid,vectorYGrid);
    end
...
odeSolverOptions = odeset(odeSolverOptions,'OutputFcn',@(time,position,flag)ode_output(time,position,flag,previousVector,vectorInterpolant,vectorXGrid,vectorYGrid,domain,flowResolution,flowPeriodicBc,checkDiscontinuity));
if isempty(poincareSection)
...
%tic;
[~,position,tEvent,~,iEvent] = ode45(@(time,position)odefun(time,position,domain,flowResolution,flowPeriodicBc,vectorInterpolant,previousVector,checkDiscontinuity,discontinuousLargeAngle,vectorXGrid,vectorYGrid),timespan,transpose(initialCondition),odeSolverOptions);
%timer1 = toc;
%disp("Integrated path in " + timer1);
...
function output = odefun(~,position,domain,flowResolution,flowPeriodicBc,vectorInterpolant,previousVector,checkDiscontinuity,discontinuousLargeAngle,vectorXGrid,vectorYGrid)

position = transpose(apply_periodic_bc(transpose(position),flowPeriodicBc,domain));

if checkDiscontinuity
    [continuousInterpolant,lDiscontinuousLargeAngle] = is_element_with_orient_discont(position,domain,flowResolution,vectorXGrid,vectorYGrid);
...
function status = ode_output(~,position,flag,vector,vectorInterpolant,vectorXGrid,vectorYGrid,domain,flowResolution,flowPeriodicBc,checkDiscontinuity)
...
if nargin < 4 || isempty(flag)
    ...
    if checkDiscontinuity
        continuousInterpolant = is_element_with_orient_discont(position,domain,flowResolution,vectorXGrid,vectorYGrid);
    end
...
% a) doesnt need to input the whole vector field at once
% b) instead of checking the orientation every time do:
% b.1) orient whole field once beforehand
% b.2) or safe results of each single orientation for next time
function [continuousInterpolant,discontinuousLargeAngle] = is_element_with_orient_discont(position,domain,resolution,vectorX,vectorY)
...
function d = drectangle(p, x1, x2, y1, y2, periodicBc)
    if periodicBc(1)
        p(1) = mod(p(1) - x1, x2 - x1) + x1;
    end
    
    if periodicBc(2)
        p(2) = mod(p(2) - y1, y2 - y1) + y1;
    end

    diff = [x1 - p(1), p(1) - x2, y1 - p(2), p(2) - y2];
    d = max(diff);
    
    % if d is negative or zero --> point inside the rectangle
    if sign(d) == 1
        dx = max([diff(1), 0, diff(2)]);
        dy = max([diff(3), 0, diff(4)]);
        d = sqrt(dx * dx + dy * dy);
    end
