...
addParameter(p,'auxGridRelDelta',1e-2,@(input)validateattributes(input,{'single'},{'scalar','>',0,'<',.5}))
...
switch method
    case 'finiteDifference'
        ...
        if eigenvalueFromMainGrid == false
        ...
        if eigenvalueFromMainGrid
            if coupledIntegration
                if Flow_Map
                    finalPositionMainGrid = squeeze(Flow_Map(:,:,3:4));
                    finalPositionMainGrid = reshape(finalPositionMainGrid, [resolution(1) * resolution(2), 2]);
                else
                    finalPositionMainGrid = ode45_vector(@(t,y)derivative(t,y,false),timespan,initialPosition,false,odeSolverOptions);
                end
            else
                npoints = prod(resolution);
                % wip
                tsteps = [timespan(1),timespan(1) + .5*diff(timespan),timespan(2)];
                finalPositionMainGrid = nan(npoints, 2, 'single');
                points_per_worker = floor(npoints / 12);
                for m = 1:12%:npoints

                    [~,yf] = ode45(@(t,y)derivative(t,y,false),tsteps, ...
                        initialPosition(((m-1) * points_per_worker + 1):(m * points_per_worker),:),odeSolverOptions);
                    finalPositionMainGrid(((m-1) * points_per_worker + 1):(m * points_per_worker),:) = yf(end,:);
                end
            end
            
            cgStrainMainGrid = compute_cgStrain(finalPositionMainGrid,initialPosition,resolution); 
            [cgStrainV,cgStrainD] = arrayfun(@(x11,x12,x22)eig_array(x11,x12,x22,customEigMethod),cgStrainMainGrid(:,1),cgStrainMainGrid(:,2),cgStrainMainGrid(:,3),'UniformOutput',false);
        end
    ...
    case 'equationOfVariation'
        ...
        [cgStrainV,cgStrainD] = arrayfun(@(a1,a2,a4)eig_array(a1,a2,a4,false),squeeze(cgStrain(1,1,:)),squeeze(cgStrain(1,2,:)),squeeze(cgStrain(2,2,:)),'UniformOutput',false);
        cgStrainV = cell2mat(cgStrainV);
        cgStrainD = cell2mat(cgStrainD);
end
...
%tsteps = linspace(tspan(1), tspan(2), 200);
%yf = ode1(odefun,tsteps,y0);
...
auxiliaryPosition = nan(size(basePosition,1),nColAuxiliaryPosition, 'single');
